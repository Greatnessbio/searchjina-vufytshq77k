...
def get_jina_search_results(query, jina_api_key):
    ...
    try:
        ...
        LOGGER.error(f"Jina AI search request failed: {e}")
        st.error("Failed to fetch Jina AI search results. Please try again later.")
    return None
...
def main_app():
    ...
    if company_url:
        ...
        # Add Jina AI search after fetching company info
        company_name = st.session_state.company_info['data'].get('companyName', '')
        if company_name:
            st.subheader("Additional Information from Jina AI")
            jina_query = f"When was {company_name} founded?"
            with st.spinner("Fetching additional information..."):
                jina_results = get_jina_search_results(jina_query, api_keys["jina"])
                if jina_results:
                    st.write(jina_results.get('answer', 'No additional information found.'))
                else:
                    st.error("Failed to fetch additional information from Jina AI.")
...
def display():
    ...
    st.caption("Note: This app uses RapidAPI's LinkedIn Data Scraper, OpenRouter for AI model access, and Jina AI for additional information. Make sure you have valid API keys for all services.")
